...
def calculate_area(sock, params):
    if "height" in params and "width" in params:
        response = str((int(params["height"]) * int(params["width"])) / 2).encode()
        sock.send(build_http_response("200", len(response), CONTENT_TYPES["txt"]))
        sock.send(response)
...
def server_loop(server_socket):
    ...
    while True:
        try:
            ...
            elif file_name == "calculate-area":
                calculate_area(client_socket, params)
            else:
